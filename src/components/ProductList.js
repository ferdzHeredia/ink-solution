...
//const jsonData = Data.map(data => data)
        ...
        <div className="p-col-12 p-md-4">
          <div className="product-grid-item card">
            <div className="product-grid-item-top">
              <div>
                <i className="pi pi-tag product-category-icon"></i>
                <span className="product-category">{jsonData.category}</span>
              </div>
              <span className={`product-badge status-${jsonData.inventoryStatus}`}>
                {jsonData.inventoryStatus}
              </span>
            </div>
            <div className="product-grid-item-content">
              <img
                src={`showcase/demo/images/product/${jsonData.image}`}
                onError={(e) =>
                  (e.target.src =
                ...
                alt={jsonData.name}
              />
              <div className="product-name">{jsonData.name}</div>
              <div className="product-description">{jsonData.description}</div>
              <Rating value={jsonData.rating} readOnly cancel={false}></Rating>
            </div>
            <div className="product-grid-item-bottom">
              <span className="product-price">${jsonData.price}</span>
              <Button
              style={{backgroundColor:"#00897b  "}}
                icon="pi pi-info-circle"
                label="Details"
                disabled={jsonData.inventoryStatus === "OUTOFSTOCK"}
              ></Button>
            </div>
          </div>
        </div>
